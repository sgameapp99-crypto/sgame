<template>
  <div class="american-football-game-card" @click="$emit('click')">
    <!-- 比賽結果區域 (僅在比賽結束時顯示) -->
...
import type { AmericanFootballGame } from '../../data/types';

interface Props {
  game: AmericanFootballGame;
}
...
function getWinnerTeam(game: AmericanFootballGame) {
...
function getMatchPeriodText(game: AmericanFootballGame) {
  ...
  if (game.status === 'live') {
    if (game.currentPeriod) {
      const periodLabels = ['第1節', '第2節', '第3節', '第4節'];
...
.american-football-game-card:hover {
